...
public final class QuadTreeOld {
	private final int MAX_OBJECTS = 10;
	private final int MAX_LEVELS = 5;

	private List<Bounds> objectBoundingBoxes = new ArrayList<Bounds>();
	private Bounds bounds;
	private QuadTreeOld parent;
	private QuadTreeOld[] children = new QuadTreeOld[4];
	private int level;

	/**
	 * Creates a new QuadTreeOld with the given maximum amount of objects per node
	 * and the bounds of this node.
	 * 
	 * @param bounds
	 *            The rectangle this node covers
	 */
	public QuadTreeOld(Bounds bounds) {
		this.parent = null;
		this.bounds = bounds;
		this.level = 0;
	}

	private QuadTreeOld(Bounds bounds, QuadTreeOld parent, int level) {
		this.parent = parent;
		this.bounds = bounds;
		this.level = level;
	}
	 ...
	 */
	public QuadTreeOld getRoot() {
		QuadTreeOld root = this;
	...
	/**
	 * Inserts a rectangle into the QuadTreeOld.
	 * 
	 * @param bounds
	...
	private void split() {
		...
		children[0] = new QuadTreeOld(new BoundingBox(x, y, halfWidth, halfHeight),
				this, level + 1);
		children[1] = new QuadTreeOld(new BoundingBox(x + halfWidth, y, halfWidth,
				halfHeight), this, level + 1);
		children[2] = new QuadTreeOld(new BoundingBox(x + halfWidth, y + halfHeight,
				halfWidth, halfHeight), this, level + 1);
		children[3] = new QuadTreeOld(new BoundingBox(x, y + halfHeight, halfWidth,
				halfHeight), this, level + 1);
	}
	...
	/**
	 * Removes an object, and returns the QuadTreeOld it was deleted from.
	 * 
	 * @param object
	 *            The object that is to be deleted.
	 * @return The QuadTreeOld it was deleted from. Null if the object does not
	 *         exist in the QuadTreeOld.
	 */
	public QuadTreeOld remove(Bounds object) {
